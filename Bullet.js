...
Bullet.listener = {
  collideSelf(){
    const dist = Math.sqrt((this.xabs - this.ref.self.locate().x) *
                           (this.xabs - this.ref.self.locate().x) + 
                           (this.yabs - this.ref.self.locate().y) * 
                           (this.yabs - this.ref.self.locate().y)
    )
    const result = (dist < (this.radius + this.ref.self.locate().radius) )
    return { dist, result }
  },
  border() {
